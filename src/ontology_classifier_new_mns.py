...
if __name__ == '__main__':
    ...
    fieldnames = ['parentproductid', 'id', 'productsku', 'producturl','fullimageurl','product_info','title', 'predicted_ontology']
    ...
    c = ''
    for entry in input1:
        if entry['mns_product_type']=='Clothing':
            output = {}
            print "{} completed".format(x)
            x += 1
            entry['title']=''
            for info in entry['product_info'].split(';'):
                if('#Product Style:' in info):
                    entry['title']=info.split(':')[1]
            if not entry['title']:
                output["product_info"] = entry["product_info"]
                output["title"] = ''
                output["parentproductid"] = entry["parentproductid"]
                output["id"] = entry["id"]
                output['productsku'] = entry['productsku']
                output['producturl'] = entry['producturl']
                output['fullimageurl'] = entry['fullimageurl']
