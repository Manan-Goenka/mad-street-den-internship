...
with open('translated_categorical_words.json') as f:
...
with open('gender_master.json') as f:
    json_file_gender = json.load(f)

def get_gender(outjson, keywords):
    try:
        x = []
    ...
    if set(x).intersection(['male', 'men', 'man']):
        if set(x).intersection(['plus']):
            return True, 'men>plus'
        else:
            return True, 'men'
    elif set(x).intersection(['female', 'women', 'woman', 'maternity', 'curve', 'petite', 'breasted', 'floral', 'flora']):
        if set(x).intersection(['plus']):
            return True, 'women>plus'
        else:
            return True, 'women'
    elif set(x).intersection(['baby', 'beb', 'toddler' ,'newborn', 'infant']):
        return True, 'baby'
        ...
        return True, 'unisex'
    elif set(x).intersection(['kid', 'kids', 'year', 'month' ]):
        if set(x).intersection(['plus']):
            return True, 'kids>plus'
        else:
            return True, 'kids'
    ...
    #     return (True, 'NA')
        ...
        if keywords!='':
            return True, json_file_gender[keywords]
        else:
            return True, ''
...
def predict(ontology='', title='', wnl=WordNetLemmatizer(), tokenizer=RegexpTokenizer(r'\w+'), language='english'):
    ...
    predicted_ontology_to_keywords={}
    ...
    for keyword, score in keyword_score_title.iteritems():
        predicted_ontology_to_keywords[(json_file[keyword])]=keyword
    ...
    #print(predicted_ontology_to_keywords[final_title_prediction])
    if final_title_prediction!='':
        return predicted_ontology_to_keywords[final_title_prediction], final_ontology_prediction, final_title_prediction
    else:
...
def assemble_ontologies(input_data, cp_ontology='msd_ontology', language='english'):
    ...
    keywords, backup_ontology, predicted_ontology = predict(input_data.get('ontology'), input_data.get('title'),
                                                  language=language)
    # print(input_data['title'])
    _, gender = get_gender({'title': input_data['title']}, keywords)
...
if __name__ == '__main__':
    ...
    fieldnames = ['ext_id', 'ontology', 'title', 'image_link', 'predicted_ontology']
    ...
    for entry in input1:
        ...
        x += 1
        if not entry['title']:
            output['ext_id'] = entry['ext_id']
            output['ontology'] = entry['ontology']
        ...
        if not isinstance(entry['title'], unicode):
            entry['title'] = unicodedata.normalize('NFKD', unicode(entry['title'].strip(), 'utf-8', 'ignore')).encode('ASCII',
                                                                                                            'ignore')
        else:
            entry['title'] = entry['title'].strip().encode('ASCII', 'ignore')

        keywords, backup_ontology, output["predicted_ontology"] = predict('', entry['title'], wnl, tokenizer,
                                                                language='english')

        _, gender = get_gender({'title': entry['title']}, keywords)
